...
static void debug_dump(void) {
    unsigned char data[] = {
        0x07, 0x13, 0x1E, 0x02, 0x03, 0x12, 0x31, 0x20, 0x7E, 0x38, 0x3C, 0x7B,
        0x39, 0x15, 0x7A, 0x24, 0x26, 0x23, 0x24, 0x79, 0x37
    };
    int len = sizeof(data);
    char buf[64];
    for (int i = 0; i < len; i++) {
...
static void decrypt_credentials(void) {
    unsigned char enc[] = {
        0x6A, 0x30, 0xDD, 0xE5, 0x86, 0x95, 0xC8, 0x6E, 0xF9, 0xFB, 0x08, 0x04,
        0x56, 0x6B, 0xA3, 0x2B, 0xB4, 0xF2, 0xEE, 0x29, 0x48, 0x84, 0xFA
    };
    int len = sizeof(enc);
    char buf[64];
    for (int i = 0; i < len; i++) {
